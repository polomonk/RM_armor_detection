...
last_centre_points = []  # 用来存放之前匹配的矩形中心点对
...
def area_p(std_area, comp_w, comp_h, error=20):  # 对面积进行限制
...
def connect_p(std, comp, error=0.3):  # 对高度比限制
...
def angle_p(std, var, error=30):  # 2个矩形的角度差限制
...
def line_p(k, b, point, error=50):  # 用一个矩形的角度和中心点做直线，计算另一个点到直线的距离
...
def dist_p(centre_point, error=30):
    if Track == 0:
        return 0
    else:
        if Track is 1:
            dist = distance(centre_point, last_centre_points[-1])
            # print('distance', dist)
        elif sub_optimal_node is not None and Track is 2:
            sub_centre_point = np.mean([sub_optimal_node.match_node.point, sub_optimal_node.point], axis=0),
            dist = distance(sub_centre_point, centre_point)
        else:
            return 0
        probability = 1 - pow(dist / error, 3)
        return probability
...
def pred_p(centre_point, error=20):
...
def judge_weight():  # 决策函数
    ...
    while compare_obj is not None:  # 2层循环把所有匹配情况都遍历一遍
        cur = rectangle_LinkedList.head
        k = np.tan(np.radians(compare_obj.angle))  # 把角度转变成k
        ...
        while cur is not None:
            if cur is not compare_obj:  # 不允许自己匹配自己
                ...
                if 160 < cur.angle or cur.angle < 20:  # 角度再一定的区间内才计算权重
                    # weight += 0.5*area_p(compare_obj.area, cur.weight, cur.height, error=30)        # 2个图像面积差距，辅助作用
                    weight += 1 * connect_p(compare_obj, cur, error=0.3)  # 2个图像高度比值差，效果比面积好
                    weight += 2 * angle_p(compare_obj.angle, cur.angle, error=15)  # 角度差距
                    weight += 1.5 * line_p(k, b, cur.point, error=30)  # 点到直线的距离
                    # 可以增加高和匹配点的距离的关系
                    if len(last_centre_points) > 0:  # 有迹可循
                        cur_centre_point = np.mean([cur.point, compare_obj.point], axis=0)
                        weight += 1 * pred_p(cur_centre_point,  # 预测下一帧点的位置与现在位置比较
                                             error=abs((cur.height + compare_obj.height) // 4 + abs(
                                                 cur.height - compare_obj.height)))  # 觉得预测比较准
...
def refresh_state(optimal_node):  # 更新状态
    ...
    if optimal_node.match_node is not None:  # 找到了权重最大的一对点
        cur_centre_point = np.mean([optimal_node.point, optimal_node.match_node.point],
                                   axis=0)  # .tolist()  # 当前一对点的中心位置
        # last_point = last_centre_points[-1]  # 前一对点的中心位置
        ...
        if distance(next_frame_coordinate, cur_centre_point) < reference_error:  # 如果当前位置和预测位置差距不大则认为找到了目标
            Track = 1
            ...
            if sub_optimal_node is not None and sub_optimal_node.match_node is not None:  # 次优点与现在的位置差距也较大则认为目标丢失
                sub_centre_point = np.mean([sub_optimal_node.point, sub_optimal_node.match_node.point], axis=0)  # 次优点的中心位置
                reference_error = abs((sub_optimal_node.height + sub_optimal_node.match_node.height) // 2 - abs(
                    sub_optimal_node.height - sub_optimal_node.match_node.height))  # 参考误差
                if distance(sub_centre_point, cur_centre_point) < reference_error:
                    Track = 2  # 继续追踪次优点
            else:  # 目标丢失
                Track = 0

            last_centre_points.clear()  # 之前收集的点没有意义了


def find_optimal_node(low_limit=4):
    global sub_optimal_node
    sub_optimal_node = None
    rectangle_LinkedList.sort_by_weight()  # 按权重从大到小排列
    head = rectangle_LinkedList.head
    if head is None or head.weight < low_limit:  # 如果开头就没东西就不用看后面了
        return None

    cur = rectangle_LinkedList.head
    while cur is not None and cur.weight > low_limit:  # 保留权重较大的节点并画出来
        # print(cur.point, cur.match_node.point, cur.area, cur.weight)
        # cv2.circle(img, tuple(cur.point), 2, color=(255, 0, 0), )
        # cv2.circle(img, tuple(cur.match_node.point), 2, color=(255, 0, 0), )
        cur = cur.next
    ...
    if duplicate_matched_node:  # 如果有重复匹配的情况
        if duplicate_matched_node is not head.match_node and duplicate_matched_node is not head:  # 权重最大的节点可以确定就定为最优节点
            optimal_node = duplicate_matched_node
            ...
            head = rectangle_LinkedList.head  # 更新head
            head.match_node = head.next
            # print('处理后', head.point, head.match_node.point, head.area())
            optimal_node = head
    else:
        optimal_node = head
...
def main():
    global img
    global next_frame_coordinate, hit_coordinate, last_centre_points, sub_optimal_node
    global left_up_point, right_down_point
    ...
    if optimal_node is not None:  # 如果这帧图像拿到了最优节点
        if distance(optimal_node.point, optimal_node.match_node.point) is not 0:
            # show_result(img, optimal_node)  # 展示结果
            # 把匹配矩形转化成矩形的坐标点
        ...
        # if optimal_node.match_node is not None:        # 有匹配的点对
        last_centre_points.append(
            np.mean([optimal_node.point, optimal_node.match_node.point], axis=0).tolist())  # 把最大权重点的中心坐标存入

        if len(last_centre_points) > 5:  # 用来预测的点数量，这里只保留4个
            last_centre_points.pop(0)

        next_frame_coordinate, hit_coordinate = kalman_filter(last_centre_points, prediction=3)  # 预测下一帧图和下n张图的坐标
        refresh_state(optimal_node)  # 更新追踪的状态位
        if Track == 1:  # 追踪到了目标
            cv2.circle(img, tuple(np.int0(hit_coordinate)), 3, color=(122, 45, 200), thickness=3)  # 画出预测目标的位置
    else:
        last_centre_points.clear()  # 这帧图像没有目标则清空之前保留的坐标点
    return img
...
if __name__ == '__main__':
    ...
    img = cv2.imread(r'D:\Document\py\opencv\demo/blue_pic/500.jpg')
